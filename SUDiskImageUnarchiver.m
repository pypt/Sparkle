...
{		
	...
	id subpathEnumerator = [[[NSFileManager defaultManager] contentsOfDirectoryAtPath:mountPoint error:nil] objectEnumerator], currentSubpath;
	while ((currentSubpath = [subpathEnumerator nextObject]))
	{
		...
		if (![[NSFileManager defaultManager] isReadableFileAtPath:currentFullPath]) {
            continue;
        }
        
        if (! [[NSFileManager defaultManager] copyItemAtPath:currentFullPath toPath:[targetPath stringByAppendingPathComponent:currentSubpath] error:nil]) {
            goto reportError;
        }
	}
